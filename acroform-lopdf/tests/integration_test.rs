...
fn test_load_and_list_fields() {
    let doc = AcroFormDocument::from_pdf("tests/af8.pdf")
...
fn test_load_from_bytes() {
    ...
    let doc = AcroFormDocument::from_bytes(bytes)
...
fn test_fill_and_save() {
    ...
    for field in fields {
        match field.field_type {
            acroform_lopdf::FieldType::Text => {
                values.insert(field.name.clone(), FieldValue::Text("Test Value".to_string()));
                break;
            }
            _ => {}
        }
    ...
    if !values.is_empty() {
        ...
        std::fs::write("/tmp/filled_test.pdf", bytes)
            .expect("Failed to write filled PDF");
        
        // Verify we can load it back
        let filled_doc = AcroFormDocument::from_pdf("/tmp/filled_test.pdf")
            .expect("Failed to load filled PDF");
        let filled_fields = filled_doc.fields()
