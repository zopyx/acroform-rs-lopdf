...
fn test_default_value_updated_with_value() {
    ...
    let uncompressed_content =
        std::fs::read("/tmp/test_dv_uncompressed.pdf").expect("Failed to read uncompressed PDF");
    ...
    assert_eq!(
        old_value_count, 0,
        "OLD_VALUE should not appear in the filled PDF, but found {} occurrences",
        old_value_count
    );

    println!(
        "✓ Default value update test passed! Found {} occurrences of new value",
        count
    );
}
